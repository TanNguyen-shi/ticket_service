    ...
    : BaseService<IEventZoneRepository, EventZoneEntity>(unitOfWork.EventZone, TicketingTypeEnum.EventZone), IEventZoneDomainService
{
    public async Task<ResponseMessage<int>> InsertAsync(EventZoneEntity entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (result is not > 0)
                throw new Exception(DomainMessageConstants.EventZone.InsertError);

            await unitOfWork.CommitAsync(cancellationToken: cancellationToken);

            return new ResponseMessage<int>().MessageSuccess(result ?? 0, DomainMessageConstants.EventZone.InsertSuccess);
        }
        catch (Exception e)
        {
    ...
    public async Task<ResponseMessage<bool>> UpdateAsync(EventZoneEntity entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!result)
                throw new Exception(DomainMessageConstants.EventZone.UpdateError);

            await unitOfWork.CommitAsync(cancellationToken: cancellationToken);

            return new ResponseMessage<bool>().MessageSuccess(true, DomainMessageConstants.EventZone.UpdateSuccess);
        }
        catch (Exception e)
        {
            await unitOfWork.RollbackAsync(cancellationToken: cancellationToken);
    ...
    public async Task<ResponseMessage<bool>> DeleteAsync(EventZoneEntity entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!result)
                throw new Exception(DomainMessageConstants.EventZone.DeleteError);

            await unitOfWork.CommitAsync(cancellationToken: cancellationToken);

            return new ResponseMessage<bool>().MessageSuccess(true, DomainMessageConstants.EventZone.DeleteSuccess);
        }
        catch (Exception e)
        {
            await unitOfWork.RollbackAsync(cancellationToken: cancellationToken);
    ...
    public async Task<ResponseMessage<EventZoneDetailDto?>> GetByIdAsync(EventZoneGetByIdRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (result is null)
                return new ResponseMessage<EventZoneDetailDto?>().MessageWarning(DomainMessageConstants.EventZone.NotFound);

            return new ResponseMessage<EventZoneDetailDto?>().MessageSuccess(result, DomainMessageConstants.EventZone.GetDetailSuccess);
        }
        catch (Exception e)
        {
    ...
    public async Task<ResponseMessage<IEnumerable<EventZoneListDto>>> GetPagedListAsync(EventZoneGetPagedListRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new ResponseMessage<IEnumerable<EventZoneListDto>>().MessageSuccess(result ?? [], DomainMessageConstants.EventZone.GetListSuccess);
        }
        catch (Exception e)
        {
